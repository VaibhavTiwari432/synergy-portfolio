# Sustainable Augmentation Framework (SAF) / ARI — Final Master Compilation **(v2.2)**
...
**Version:** v2.2 — **interaction-dynamics integration.** Adopts the surviving layer from the AEGIS ↔ SAF cross-examination (AEGIS: independent greenfield design, §0.6.1; triage review across models, June 2026): the universal event log, event-conditioned reaction signatures with the Friction Transition Matrix, five named transition-pattern metrics, embedded micro-probes with **randomized exposure**, the partner reliability map, a **descriptive** regime overlay (explicitly not a second latent model), and the shared-gold bets protocol (§8.6). Every v2.2 item is a derived metric, an instrument, schema, or governance — **ontology untouched** (audit: §14.4). The review's summary sentence is adopted as this revision's thesis: *SAF understands cognition; AEGIS understands interaction dynamics; v2.2 is where they converge.*
**Prior revisions:** v2.1 — **the humanity revision** (population validity, fairness, field measurement, corpus design; change set §0.0a; originates from the commissioned first-principles re-derivation, §0.6.1). v2 — measurement-engine and scientific-rigor revision (change set §0.0). The **ontology has been frozen since the v2 commit** (§14.3): no new neurons, dimensions, pillars, or latent variables for the validation year.
...
**How changes are marked:** the consolidated **v2 Change Set** (§0.0), **v2.1 Change Set** (§0.0a), and **v2.2 Change Set** (§0.0b) tables state every change (what + why + section); inline **`[v2]`**, **`[v2.1]`**, and **`[v2.2]`** callouts mark each edit at its point of application.
...
## 0.0b v2.2 Change Set — interaction-dynamics integration `[v2.2]`

**Provenance.** AEGIS v1.0 (an independent greenfield rival design, written without copying) was compared against this document in a cross-model triage (June 2026). The triage question — *does this improve measurement without violating the ontology freeze?* — is the correct one and was applied correctly; v2.2 adopts the surviving items with four amendments recorded here so the reasoning cannot be lost: (i) the **event log** is the substrate every other import is computed from, so it leads the list rather than trailing it; (ii) the **partner reliability map** is not optional — v2.1's calibration slope $\hat{s}$ (§5.8d) is uncomputable without it; (iii) the HSMM is excluded for the **right** reasons — the freeze forbids a second latent state machinery and the construct "surrender" must have exactly one owner (CSPC) — not because continuous models are "more realistic"; what imports is a **descriptive overlay**; (iv) four proposed items (sequence metrics, friction signatures, transition traits, "reaction signatures") are **one mechanism** and are adopted as one: the Event-Conditioned Response layer. One risk the triage missed is named and handled: **micro-probes are interventions, not just measurements** (the testing effect) — exposure is randomized and modeled (V4, A10).

| # | Change | Why (the failure it fixes) | Section(s) | Disposition |
|---|---|---|---|---|
| V1 | **Universal event log** — append-only `[timestamp, event_type, actor, payload_ref, confidence, provenance]` for every extraction event | Order carries signal rates destroy; today's pipeline keeps aggregates and discards sequence; schema-only cost, future-proofs every v3 sequence model | §5.9a, §12 | Adopted (substrate) |
| V2 | **Event-Conditioned Response (E→R) layer** — frozen small event taxonomy × response classes → per-user conditional distributions $\pi_i(r\mid e)$, with per-cell event-count gating; flagship slice: the **Friction Transition Matrix** | Humans are defined as much by *responses to events* as by base rates; "what do they do when the AI errs / they hit friction / sources contradict" is the most trait-like signal in the sequence; consolidates four overlapping proposals into one mechanism | §5.9b–c, FC 13.13 | Adopted |
| V3 | **Five named transition-pattern metrics** (frozen list): verify-after-error rate; constraint-before-generation rate; prediction-before-answer rate; accept-run length; revision-after-output rate | The specific sequence statistics with the strongest prior diagnostic claim; derived from the event log, no new items | §5.9d, §6.4 | Adopted |
| V4 | **Embedded micro-probes** MP-1 predict-then-reveal · MP-2 teach-back close · MP-3 AI-off tasklet, with **randomized exposure** and probe-as-intervention accounting in λ | Manufactures human-alone snapshots and calibration ground truth *inside* sessions; randomization makes the testing-effect estimable instead of a contaminant | §7.5a, §9.2–9.3, §10 (A10) | Adopted |
| V5 | **Counterfactual-manufacturing principle** as design commitment 9 | Where reality does not supply the comparison a claim needs, the instrument designs it (probes, menus, AI-off arms) rather than assuming data exists | §0.4 | Adopted |
| V6 | **Partner reliability map** $\hat{R}(c, m)$ as a maintained, versioned artifact (hierarchical beta-binomial per risk-class × model-era; benchmark priors + judge-verified outcomes + MP-1 ground truth) | Trusting arithmetic and trusting medicine are different acts; $\hat{s}$ and risk-conditioned EC (v2.1 §5.8) require this map to exist as more than a classifier prior | §5.8f | Adopted |
| V7 | **Descriptive regime overlay** — rule-based segment labels {generative · extractive · verification · drift · **accept-run**} computed from observed events; occupancy shares and run lengths as derived features; explicitly **not** a latent model | Gives reports a human-readable session shape and gives bet B2 a home — without a second latent state machinery; CSPC remains sole owner of latent state and of the word "surrender" | §5.9f | Adopted (descriptive only) |
| V8 | **Shared-gold bets protocol** — the six pre-registered AEGIS↔SAF disagreements (B1–B6) adjudicated on jointly coded gold, with a commit-to-adopt clause in both directions; wholesale-import rejections logged with "losing the bet" as the re-entry condition | Converts a design rivalry into an experiment; prevents either side declaring victory by fiat | §8.6, §14.1–14.2 | Adopted (governance) |

> **Governing sentence (v2.2):** *rates describe what a person does; reactions describe who they are when it goes wrong.* v2.2 teaches the instrument to see the second without adding one element to the ontology.
...
9. **Manufacture the counterfactual.** `[v2.2]` Where reality does not supply the comparison a claim needs — a solo baseline, a pre-exposure prediction, a no-AI arm — the instrument **designs** it (micro-probes §7.5a, task menus §2.6/§9.3, AI-off arms) rather than assuming the data exists or quietly inflating what transcripts can identify. A claim without its counterfactual is not promoted up the ladder; it waits for the instrument that manufactures one.
...
> **`[v2.1]` Model-era cohort norming.** Classical psychometrics assumes a fixed test environment; here the *other member of the dyad improves every few months*. $\kappa^{AI}_m$ absorbs the model term inside the IRT, but **extraction-layer norms drift too**: as models improve, elaborate prompting becomes less necessary, verification needs shift across content classes, iteration counts compress. A fixed rubric therefore scores users *lower over time while they are getting better-calibrated* — construct drift built into the substrate. Rule: four schema fields are mandatory on every session — `model_family`, `model_version`, `platform`, `capture_date` — and **all normalizations are computed within model-era cohorts**; cross-era comparisons are flagged, never silent. The corresponding PR rubric renorm is deferred to v3 (§14.2); the schema fields are immediate because they are nearly free now and unrecoverable later.
...
> **`[v2.1]` Pre-registered degradation branch.** $A_t$ is the weakest-identified state by construction: text-derived affect is a coarse, culturally confounded proxy — many frustrated users go *terse and polite*, not angry, a pattern expected to be common in the deployment population. If $A_t$ fails the §8.1 identifiability test (posterior correlation with a neighbor → 1, or loadings indistinguishable from noise), the cascade is **re-estimated as the direct path $C_t \to M_t$**, with the affective link absorbed into transition noise. The branch is pre-registered now so a likely identifiability outcome reads as anticipated science rather than retreat. **No permitted claim at any tier depends on $A_t$ specifically**, so the branch costs nothing downstream; the surrender detector (FC 13.3) degrades gracefully — its $A_t$ checkpoint becomes a pass-through.
...
### 5.8 Verification evidence — provenance, calibration, theater, and the partner map `[v2.1]`

**(a) The off-screen problem.** Most real verification happens *outside* the transcript — the code is run, the source checked in another tab, a colleague asked. Transcript-visible EC is therefore a **censored lower bound with person-varying censoring** (engineers verify off-screen constantly; novices often not at all). This — not annotation volume alone — is the deeper reason EC calibration has lagged (§7.3).

**(b) Provenance-tagged evidence.** **[MEASURABLE]** EC evidence is routed with a provenance tag carrying a prior precision (consistent with v2's rule: evidence quality moves **precision, never score values**):
- `displayed` — verification visible in-transcript (challenge, test request, cross-check). Full prior precision.
- `implied` — observable traces of off-screen verification: (i) pasted-back error messages / run outputs; (ii) a latency gap beyond the user's rolling personal baseline (§9.2) followed by an *informed* correction or constraint; (iii) tested-state language ("fails on empty input"; "the second source disagrees"). Reduced prior precision, **upgraded** when consented Tier-2 telemetry (tab-switch, copy-out) corroborates.

All routes feed **existing EC neurons** as evidence sources; no new items (audit: §14.4).

**(c) Calibrated, not counted.** **[DESIGNED]** Verification value is **risk-conditioned**. Claims/spans are classed by a lightweight claim-risk classifier into classes $c$ with partner reliability $\hat{R}(c, m)$ (see (f)). Verification of high-risk / low-reliability content earns full EC credit; verification of trivial, high-reliability content earns little — and at volume routes to **extraneous load $C_t$**: the Over-Verifier's excess scrutiny is itself load, not quality. The construct is *calibrated scrutiny*, never scrutiny volume.

**(d) The calibration slope $\hat{s}$.** **[DESIGNED → MEASURABLE once the risk classifier ships]** Per user $i$, with acceptance rate $a_{i,c}$ over applicable claims in class $c$:

$$\hat{s}_i = \frac{\operatorname{cov}_c\big(a_{i,c},\ \hat{R}(c,m)\big)}{\operatorname{var}_c\big(\hat{R}(c,m)\big)}$$

Healthy: $\hat{s}_i > 0$ with moderate mean acceptance — reliance tracks reliability. **Uniform trust:** $\hat{s}_i \approx 0$ with high $\bar{a}$ — the most damaging wild-type pattern, previously scattered across EC/AUI. **Uniform distrust:** $\hat{s}_i \approx 0$ with low $\bar{a}$ — cost without protection. $\hat{s}$ is a **derived metric over existing extraction** (registered §6.4); whether it deserves a *named factor* is an EFA decision, pre-registered in §14.2 — under the freeze it is a metric, not a latent variable.

**(e) Verification theater.** **[DESIGNED]** Once scrutiny is scored, scrutiny can be performed. Theater is detectable as **verification acts with null downstream delta** — neither stance nor content changes and no confirmation evidence enters:
...
with $f$ monotone, functional form fit on data — a **precision discount, never a score change** (v2 #2). Together with the deferred probe (ungameable, §7.5), this keeps the residual Goodhart surface small: gaming the remaining in-session metrics mostly means *actually practicing the healthy behaviors* (§0.6.3-4). Evidence routing: FC 13.12.

**(f) The partner reliability map (the living artifact).** `[v2.2]` $\hat{s}$ and risk-conditioned credit require $\hat{R}(c, m)$ to exist as more than a classifier prior. It is maintained as a **versioned apparatus artifact** (§0.3 — apparatus, never the science): a hierarchical beta-binomial per (risk-class × model-era) cell, seeded from public benchmark priors per era, updated by judge-verified claim outcomes accumulating in-system and by MP-1 ground truth where it emerges (§7.5a). Era keys come from the §3.7 schema fields. Validity is itself a named assumption (A11, §10): the map is back-tested against verified error rates each era, and demoted if its ordering stops predicting them.

### 5.9 The Event-Conditioned Response layer — reaction signatures `[v2.2]`

The question "what behaviors occurred?" misses the most trait-like signal in a session: **what does this person do when something happens?** The same friction event sends one user into verification and another into an accept-run — and that fork, repeated across sessions, is closer to *who they are as a collaborator* than any base rate. v2.2 instruments this as one mechanism, consolidating what arrived as four separate proposals.

**(a) The substrate — the universal event log.** **[MEASURABLE, schema-only]** Every extraction event is appended, in order, to:

```json
event_log: [ { "t": "...", "event_type": "...", "actor": "human|ai|system",
               "payload_ref": "...", "confidence": 0.0, "provenance": "displayed|implied" } ]
```

Neuron firings, state flags, probe events, AI-error detections, contradictions — everything, timestamped, sequence-preserved. The pipeline currently aggregates and discards order; the log retains it. It costs schema and storage only, and it is the substrate from which (b)–(f) — and any v3 sequence model — are computed. *Order carries what rates destroy.*

**(b) The event taxonomy (frozen, small).** Six trigger events, drawn predominantly from the **state-diagnostic channel** and system detections: `E-ERR` (AI error detected — judge-flagged or user-evidenced), `E-CONTRA` (contradiction: AI-internal, or AI vs user-supplied source), `E-CONFUSE` (user confusion marker), `E-FRICTION` (task failure / error return / blocked progress), `E-CORRECT` (the AI corrects the user), `E-OVERREACH` (unsolicited scope expansion / sycophancy marker).

**(c) Reaction signatures.** **[DESIGNED → MEASURABLE]** For each event, open a response window of $k$ human turns and classify the first substantive response into classes drawn from the **trait channel**: `VERIFY/CHALLENGE`, `SYNTHESIZE`, `CONSTRAIN`, `ACCEPT-FLAT`, `DISENGAGE`, `DELEGATE-MORE`. Per user, estimate the conditional distribution $\pi_i(r \mid e)$ with Dirichlet partial pooling toward the population; **every (e, r) cell is gated on event count** ($n_e \ge$ threshold, else N/A — the §7.1a discipline applied to events, not turns). The flagship slice is the **Friction Transition Matrix**: $P(\text{VERIFY} \mid \text{E-FRICTION})$ vs $P(\text{ACCEPT-FLAT or DISENGAGE} \mid \text{E-FRICTION})$ — the single contrast with the strongest prior claim to predicting the deferred probe. **Partition note:** events are state-channel observables, responses are trait-channel behaviors; conditioning the second on the first *is* state-conditioned interpretation (§4.7) and preserves the v2 partition — no feature is double-counted, and the SSM remains the sole owner of latent state.

**(d) The five named transition-pattern metrics (frozen list).** **[MEASURABLE from the log]** (1) **verify-after-error rate** — share of `E-ERR` followed by `VERIFY/CHALLENGE` within window; (2) **constraint-before-generation rate** — share of generation requests preceded by user-injected constraints; (3) **prediction-before-answer rate** — share of substantive asks preceded by a stated expectation (spontaneous, or MP-1-evidenced); (4) **accept-run length** — max/mean consecutive substantive acceptances with zero scrutiny events; (5) **revision-after-output rate** — share of AI outputs the user materially transforms before use. All five are derived metrics over existing neurons and the log; none is a new item.

**(e) Where they flow.** Reaction signatures and transition metrics enter as **evidence for existing dimensions** (EC, CA, CS, AUI per the contract table's evidence-source fields) at provenance-tagged precision, and as features for persona description — never as new latent variables.

**(f) The descriptive regime overlay.** `[v2.2]` **[MEASURABLE, rule-based]** For reporting and for sequence features, sessions are segmented by deterministic rules over the log into labels: **generative** (synthesis/constraint/prediction-dense), **extractive** (ask-receive runs), **verification** (scrutiny-dense), **drift** (low task closure, topic wander), **accept-run** (consecutive unscrutinized acceptances). Outputs: occupancy shares, run-length distributions, and a color-coded session strip in reports. Three guardrails, stated bluntly: the overlay is **not a latent model** (no inference, no hidden states — rules over observed events); **CSPC remains the only latent state machinery** in the instrument; and the overlay's `accept-run` label is a *behavioral description*, deliberately not named "surrender" — **surrender is a CSPC construct** ($M_t$ collapse) with exactly one owner. The overlay exists for legibility and to host bet **B2** (§8.6): if sequence/transition features beat HGF state summaries at predicting the probe, that finding is adopted; if not, the overlay stays cosmetic. Event flow: FC 13.13.
...
### 6.4 The computed behavioral metrics — three implemented, others specified `[v2.1]` `[v2.2]`

Deterministic/semantic metrics underpin specific neuron families (the first three are implemented; the rest are specified with their section of record):
...
- **calibration_slope $\hat{s}$** `[v2.1]` — acceptance-vs-claim-risk regression per user (§5.8d); the operational proxy for trust calibration (Lee & See, 2004). **[DESIGNED]** pending the claim-risk classifier; computed from existing acceptance/verification extraction — no new neuron.
- **The five transition-pattern metrics** `[v2.2]` — verify-after-error, constraint-before-generation, prediction-before-answer, accept-run length, revision-after-output (§5.9d); computed from the event log. **[MEASURABLE once the log ships.]**
...
### 7.2a Archetype quota sampling — the contrast-case corpus design `[v2.1]`

The four-session calibration's most important finding was methodological: a single behavioral archetype produces near-zero discriminant variance — **volume without contrast is uninformative**. The gold-corpus target is therefore restated from a count to a **design**, sampled across the 10-archetype frame of §1.5:

| Quota rule | Spec |
|---|---|
| Coverage | ≥ 6 gold chats per archetype cell (≥ 60 total) |
| The twin pair | ≥ 10 each: Compressed Expert, Delegating Manager — powers the §8.5 gate |
| Verifier-rich rows | Verifier-Engineer + Anxious Over-Verifier cells double as the EC few-shot corpus (§7.3) |
| Language slice | ≥ 20% code-switched (Hindi–English) or Hindi-dominant, reflecting the deployment population; grounds A8 and §7.7a stage-3 DIF |
| Out-of-scope cell | ≥ 4 Companion-Seeker chats annotated as **intent-classifier gold** (§6.2a), not dimension gold |
| Residual check | After quota fill, cluster the residuals; unclaimed clusters extend the **sampling frame** (A7) — the frame is a tool and is not frozen; the ontology is |

Recruitment note: a hackathon population can be **task-designed into cells** (timed extraction sprints, open synthesis briefs, verification challenges, menu sessions), converting an event into a stratified corpus — the cheapest contrast-case source available.

### 7.3 The EC weak spot (named honestly)

Error Correction (EC) is the highest-signal dimension for Skilled-Outsourcer detection and the current calibration weak spot: **EC MAE ≈ 0.41**, above the per-dimension target. The fix is **few-shot intervention**, which requires a **40+ gold-corpus** before it can resume. Until then EC scores carry a wider credible interval. **`[v2.1]`** The 40+ requirement is absorbed by the archetype quota (§7.2a), whose verifier-rich cells supply the EC-dense contrast cases few-shot curation needs; and the off-screen censoring named in §5.8a is the construct-level reason volume alone was never going to close the gap.
...
1. **48-hour probe `[v2.1: format corrected]`** — lightweight unaided **generative recall** (free recall / teach-back: *"state the core argument you made two days ago, in your own words, no notes"*), gamified for participation. **Rapid product/feedback signal.** Catches surface forgetting. **Recognition formats are excluded:** the testing-effect literature (Roediger & Karpicke, 2006; Bjork's desirable difficulties) shows recognition is far less sensitive to consolidation differences than generative retrieval — a recognition-style 48-h probe risks a **false null** against the d ≈ 0.68 deferred benchmark and would mislead the product loop. Rubric family owned by the psychometrics lead (Q\* line, §12).
...
### 7.5a Embedded micro-probes — manufacturing baselines inside sessions `[v2.2]`

**[DESIGNED]** Commitment 9 made operational. Three tiny instruments, each manufacturing a comparison that observation alone cannot supply:

| Probe | What happens | What it identifies | Cost |
|---|---|---|---|
| **MP-1 Predict-then-reveal** | On sampled turns (~1 in 12, adaptive), before the AI answers: a one-line prediction + confidence (1–5) | A human-alone signal on real tasks; **ground truth for the metacognitive calibration gap** (Brier of stated confidence vs revealed correctness); feeds the partner map where correctness emerges | Seconds |
| **MP-2 Teach-back close** | At sampled session ends: *"state the core thing you'd take from this, your own words, no scrolling"* | Immediate encoding, generatively (the 48-h probe's in-session sibling) | 60–90 s |
| **MP-3 AI-off tasklet** | A short matched task with the AI visibly disabled | A solo-θ snapshot; the within-person baseline that lets λ be estimated as a trend rather than waited for | 3–5 min, weekly cadence, framed as a streak |

**Tier mapping:** Tier 1 — none (transcript-only). Tier 2 — MP-1/MP-2 as consented widgets, MP-3 opt-in. Tier 3 — all three, scheduled. MP responses are inputs to calibration and λ machinery; like self-ratings, **they never blend into behavioral dimension scores.**

**Probe-as-intervention accounting (the caveat the import review missed).** Prediction and retrieval are themselves learning events — the testing effect means MP-1/MP-2/MP-3 plausibly *improve* the retention they help measure. Untracked, that contaminates λ. Therefore: **probe exposure is randomized** (at session level for MP-1/2, person-period level for MP-3) and entered as a covariate in the λ model (A10, §10). If probes causally improve retention, that is reported as an **intervention finding** — a desirable-friction discovery in its own right — never laundered into measurement.

**Claims-ladder effect, stated precisely.** MP-1 upgrades the metacognitive calibration gap from *inferred* to *measured* at Tier 2, and MP-3 supplies a labeled **solo-θ trend proxy**. Neither unlocks "true synergy" below Tier 3: there is still no AI-alone arm, and a one-line prediction is not full solo task performance. The charter (§9.4) changes only where this section says it does.
...
### 8.5 The twin-discrimination gate `[v2.1]`

**[DESIGNED → pre-register before the first gold chat is scored]** §8.2 falsifies the human signal; §8.3 tests prediction against the probe; §8.5 stakes the instrument's hardest **discrimination** job. The **Compressed Expert** and the **Delegating Manager** (§1.5) produce near-identical surface efficiency — terse prompts, high acceptance, fast sessions — with **opposite** sustainability. Every naive efficiency metric confuses them, and the two error directions are differently expensive: false pathology on experts is a **fairness harm**; false health on outsourcers is a **validity failure**.

**Test:** on held-out, never-trained gold (§7.2a quota cells, ≥ 10 per archetype), classify CE vs DM from the full feature/dimension set. **Gate:** AUC ≥ $\tau_{\text{twin}}$, frozen at pre-registration. **Proposed defaults pending team sign-off: floor 0.70, target 0.80.** Expected load-bearing discriminators — recorded as a prediction, not a constraint: prompt information density, correction sharpness when the AI errs, calibration slope $\hat{s}$ (§5.8d), Attribution Gap, and the Friction Transition Matrix (§5.9c); the classifier is free to use all evidence. **Failure consequence:** the Skilled-Outsourcer flag (Tier 2 permitted claim, §9.2) is **suspended for terse-profile users** until the gate passes — the claims charter contracts to fit demonstrated discrimination, exactly as commitment 8 requires.

### 8.6 The shared-gold bets protocol — adjudicating the AEGIS disagreements `[v2.2]`

**[Governance]** AEGIS v1.0 (the independent greenfield design, §0.6.1) and SAF disagree on six pre-registerable measurement strategies. Rather than import-by-argument or reject-by-fiat, the disagreements are **adjudicated empirically on jointly coded gold** (the moves↔neurons crosswalk seed exists in AEGIS Appendix C):
...
| B2 | HGF continuous states condition precision | Transition/sequence features carry the signal | ΔAUC predicting the deferred probe: HGF state summaries vs §5.9 sequence features (the overlay hosts the SAF side) |
| B3 | Calibration distributed across EC/AUI | Calibration as a separable factor | Does a calibration factor emerge in EFA with unique predictive variance? (pre-registered, §14.2) |
...
**Commit-to-adopt clause, both directions:** the losing mechanism is adopted from the winner; the §14.1 wholesale-import rejections carry "losing the relevant bet" as their re-entry condition. This converts a design rivalry into an experiment — which is what pre-registration is for.
...
- **Forbidden claim:** true synergy (no solo θ baseline), direct cognitive-load measurement (only inferred proxy), competence-vs-surrender discrimination (a quiet transcript looks identical for mastery and surrender), any sustainability statement (one chat, no probe). **Never the word "synergy."** **`[v2.1]`** Any statement about the user's overall delegation *policy*: per §2.6, transcripts observe only brought-tasks, so AUI reads as **within-chat delegation choices** — nothing wider.
...
- **Permitted claim:** *AI-collaboration quality + within-session trajectory.* Detects the Skilled Outsourcer who ships strong output through blind delegation; rewards verification, iteration, independent constraint injection. **`[v2.1]`** AUI carries the same restriction as Tier 1 (§2.6: within-chat delegation choices only); the optional stakes/pressure widget feeds **context covariates only** — never behavioral scores (the self-rating rule). **`[v2.2]`** MP-1/MP-2 ship here as consented widgets with **randomized exposure** (§7.5a); MP-3 is opt-in. **`[v2.1]`** The Skilled-Outsourcer flag is **suspended for terse-profile users unless the §8.5 twin gate has passed.**
...
- **`[v2.1]` The task-menu design (the AUI unlock):** the platform presents a menu of matched tasks and lets the participant choose which to attempt solo versus with AI; the choice pattern identifies the allocation policy $\pi_i$ on the menu (§2.6) — the only clean measurement of selectivity the architecture admits. Stakes/pressure intake (two items) is recorded per session as covariates. **`[v2.2]`** All three micro-probes run scheduled here; probe-exposure randomization is the platform's responsibility and enters the λ model as a covariate (A10).
...
The governing sentence: **the extension says *collaboration quality*, never *synergy*; the platform says *synergy and sustainability only after the gate fires*, and *formative before summative* for any minor.** **`[v2.1]`** And the per-dimension fine print: **AUI reads as within-chat delegation choices below Tier 3**, and **the Skilled-Outsourcer flag is suspended for terse-profile users unless §8.5 has passed.**

### 9.5 Data dignity & the minors' reporting rule `[v2.1]`

Operationalizing §0.6.3 item 3 — dignity as architecture, not policy text:
...
- **Consent:** tier-appropriate and plain-language (the translation layer, commitment 7, applies to consent documents too); guardian consent for minors; DPDP compliance is a build deliverable with a named owner (§12, Δ9), not a paragraph.
- **Reporting to minors:** band + behavioral attribution + one next habit. **No bare composite, no percentile, no peer rank** — population norms never leak into a leaderboard (§7.7a-4 restated as a reporting rule).
- **No third-party sale or transfer, ever;** research sharing is de-identified and aggregate only, post-consent.
...
| 5 | The cascade $C_t→A_t→M_t$ is causal, not merely correlational | Held by theoretical argument + Tri-System Theory | **Sign-constrained estimation** of $\mathbf{F}$ off-diagonals + identifiability test (§8.1) across users. Pre-registered fallback: the $C_t \to M_t$ direct path (§4.3). |
| 6 | `[v2.1]` Implied-verification traces validly index off-screen verification | Held by face validity (error-paste, informed return after a latency gap) | **Tier-2 concurrent validity**: telemetry-corroboration rate + a think-aloud subsample (n ≥ 20). Falsified if implied-EC fails to track corroborated verification (pre-registered threshold, e.g. r < 0.3) → `implied` provenance demoted to zero prior precision. |
| 7 | `[v2.1]` The 10-archetype frame spans the deployment population | Held from large-scale informal observation (source stated, §1.5) | **Residual clustering** after quota fill (§7.2a); unclaimed clusters extend the frame. The sampling frame is explicitly not frozen. |
| 8 | `[v2.1]` Measurement invariance holds across language modes (English / Hindi / code-switched) | **Unknown — named as a validity threat, not assumed** | **DIF / invariance testing** (§7.7a stage 3) on the ≥ 20% code-switched gold slice. Consequential use for any group is blocked until invariance is shown for that group. |
| 9 | `[v2.1]` Reactivity is net-benign for the core healthy-behavior metrics; theater is detectable | Held by design argument (gaming ≈ practicing) + the ungameable probe | **TR audit** (§5.8e) + a disclosure A/B at Tier 3 (scored-aware vs scored-blind arms). Falsified if aware-arm scores inflate without matching probe gains. |
| 10 | `[v2.2]` Micro-probe effects on retention are estimable (probes are interventions) | Held by design: exposure is **randomized** (§7.5a) | Probe-exposure covariate in the λ model; if MP probes causally improve retention, report as an intervention finding, never as measurement. |
| 11 | `[v2.2]` The partner reliability map $\hat{R}(c,m)$ tracks true model reliability | Seeded from benchmark priors; updated by verified outcomes | **Per-era back-test**: the map's ordering must predict judge-verified error rates; demoted (and $\hat{s}$ suspended) for any era where it fails. |
...
**`[v2.1]` `[v2.2]` Build deltas (immediate, all freeze-safe, ordered by leverage-per-rupee):**

| Δ | Item | Cost | Why now |
|---|---|---|---|
| Δ1 | Schema: `model_family`, `model_version`, `platform`, `capture_date` on every session (§3.7) | Hours | Unrecoverable retroactively |
| Δ2 | Schema + intake: stakes & pressure items — Tier 3 mandatory, Tier 2 optional widget (§2.6) | Hours | Confound removal for λ and archetype assignment |
| Δ3 | **Universal event log** (§5.9a) — append-only, sequence-preserving | Hours–days | The substrate for Δ7–Δ8 and every v3 sequence model |
| Δ4 | 48-h probe switched to generative recall (§7.5) | Days | False-null risk on the product loop |
| Δ5 | Gold-corpus plan rewritten to archetype quota (§7.2a); hackathon tasks designed into cells | Days | The variance-collapse fix |
| Δ6 | **Pre-registration package filed** (gates §8.2, §8.3, §8.5; $\tau_{\text{twin}}$; the §8.6 bets; the null-result pledge) — e.g. OSF | Days | Credibility that cannot be retrofitted |
| Δ7 | Claim-risk classifier + **partner reliability map seed** (benchmark priors, current era) (§5.8c, f) | 1–2 wks | Unlocks $\hat{s}$ and risk-conditioned EC; twin-gate discriminator |
| Δ8 | E→R taxonomy + Friction Transition Matrix + the five transition metrics (§5.9) | 1–2 wks | The reaction-signature layer; B2's SAF-side features |
| Δ9 | DPDP / data-dignity implementation per §9.5, named owner | Sprint | Precondition for any minor-facing pilot |
| Δ10 | MP-1/MP-2 widgets + MP-3 tasklet bank (seed ~20 items) with **randomized exposure** (§7.5a) | Sprint | Manufactures the counterfactual; A10's design requirement |
| Δ11 | ES trigger taxonomy + event-conditioned reporting (§2.7) | Days | Stops noise-averaging on the N/A-heaviest dimension |
| Δ12 | Provenance-tagged EC evidence routing + TR computation (§5.8, FC 13.12) | 1–2 wks | EC validity; theater instrumentation |

Ownership: architecture review — Ritesh; probe rubrics + DIF — psychometrics lead; pre-registration filing — project lead.
...
### 13.12 EC evidence provenance & precision routing `[v2.1]`
*Trigger: any verification-relevant event in a turn (displayed act, error-paste, latency-gap informed return).*

```mermaid
flowchart TD
    EV["Verification-relevant event"] --> PR{"Provenance?"}
    PR -->|"displayed in-transcript"| D["Tag: displayed<br/>full prior precision"]
    PR -->|"implied trace"| I["Tag: implied<br/>reduced prior precision"]
    I --> TEL{"Tier-2 telemetry<br/>corroborates?"}
    TEL -->|"yes"| UP["Upgrade precision"]
    TEL -->|"no"| KEEP["Keep reduced"]
    D --> RISK{"Claim-risk class of<br/>verified content — R-hat(c, m)"}
    UP --> RISK
    KEEP --> RISK
    RISK -->|"high-risk / low-reliability"| FULL["Full EC credit"]
    RISK -->|"trivial / high-reliability"| CT["Minimal credit;<br/>at volume routes to C_t"]
    FULL --> THE{"Downstream delta null?<br/>(theater check)"}
    CT --> THE
    THE -->|"null — nothing changed"| TR["Increment TR;<br/>discount EC evidence precision"]
    THE -->|"stance or content changed"| OK["Evidence enters EC neurons<br/>at assigned precision"]
    TR --> OK2["Evidence enters at<br/>discounted precision"]
```

### 13.13 Event-conditioned response update (reaction signatures) `[v2.2]`
*Trigger: any taxonomy event lands in the event log (§5.9b).*

```mermaid
flowchart TD
    EVS["Extraction event stream"] --> LOG["Append to event_log<br/>(t, type, actor, conf, provenance)"]
    LOG --> CLS{"Event in E-taxonomy?<br/>E-ERR / E-CONTRA / E-CONFUSE /<br/>E-FRICTION / E-CORRECT / E-OVERREACH"}
    CLS -->|"no"| PASS["Stored for sequence metrics only"]
    CLS -->|"yes"| WIN["Open response window:<br/>next k human turns"]
    WIN --> RESP{"First substantive response class<br/>(trait channel)"}
    RESP --> PI["Update pi_i(r | e)<br/>Dirichlet partial pooling"]
    PI --> GATE{"Cell event count<br/>n_e >= minimum?"}
    GATE -->|"no"| NA["Cell = N/A<br/>(7.1a discipline on events)"]
    GATE -->|"yes"| OUT["Reaction signature + Friction<br/>Transition Matrix features<br/>to trait evidence, precision-tagged"]
    OUT --> OVR["Descriptive regime overlay strip<br/>(report layer; not latent)"]
```
...
| **Wholesale alphabet swap (107 neurons → 36 moves)** `[v2.2]` | Not adjudicable by argument; the neuron bank is load-bearing for the entire calibration state. Rival measurement strategies are settled empirically, not by fiat. | **Losing bet B1** (§8.6) on jointly coded gold. |
| **A second latent state machinery (HSMM regimes alongside the HGF)** `[v2.2]` | The freeze forbids new latent variables; two state models means two owners of "surrender" — construct duplication. (Note: the criterion is identifiability and governance, *not* "continuous is more realistic.") The **descriptive overlay** (§5.9f) imports the legible part with zero latent cost. | **Losing bet B2** + freeze lift. |
| **Collapsing 8 dimensions into 5 pillars** `[v2.2]` | Structure is discovered, not decreed (commitment 6); pre-empting EFA repeats the R1 error from the other direction. | **B6 / EFA outcome** (§7.7). |
...
| **Calibration slope $\hat{s}$ as a *named factor*** `[v2.1]` | Computed now as a derived metric (§5.8d); naming it a latent factor would add an ontology element. | **EFA** (§7.7): if a calibration factor emerges from the 107-neuron matrix, it is named then (bet B3). |
| **PR model-era rubric renorm** `[v2.1]` | Construct drift is real (§3.7) but a renorm needs ≥ 2 eras of data. | v3; the §3.7 schema fields collect from now. |
| **ES dimension-status review** `[v2.1]` | Event-conditioned ES (§2.7) may not recover as a factor — rare, domain-skewed events. | Post-EFA: retain as a factor, or re-report as an event-conditioned index. |
| **Far-transfer probe item** `[v2.1]` | The near-transfer deferred probe ships; far-transfer is the stronger schema test but costs design and cohort time. | Budget/cohort permitting at Tier 3. |
| **Sequence embeddings over the event log** `[v2.2]` | Powerful but computationally heavy and uninterpretable before the named metrics (§5.9d) have been validated. | v3, after the five transition metrics earn their keep. |
| **Any latent sequence model (HSMM / switching SSM)** `[v2.2]` | See the §14.1 rejection — latent duplication; the descriptive overlay carries the legibility for free. | Post-validation freeze lift **and** bet B2 showing sequence signal the HGF summaries miss. |
...
### 14.4 Freeze-compliance audit — v2.1 and v2.2 `[v2.1]` `[v2.2]`

Every change in §0.0a and §0.0b was audited against §14.3 before adoption. Totals across both revisions: **neurons added: 0 · dimensions added: 0 · pillars added: 0 · latent variables added: 0.** Everything lands inside the freeze's permitted list (new *fields* on existing neurons, measurement-engine fixes, calibration work, corpus collection, validation, governance).

| Change group | Items | Layer touched | Ontology impact |
|---|---|---|---|
| Charter & governance | H1, H17, H18, V5, V8 | Governing principles, reporting rules, pre-registration | None |
| Sampling & corpus | H2, H13, H14 | Recruitment frame, annotation policy, gold composition | None — the sampling frame is a tool, not ontology |
| Gates & pre-registrations | H3, H11, B1–B6 | §8 falsification machinery | None |
| Schema & covariates | H4, H5, H6, V1 | Data model, intake items, event log | None — fields and instruments |
| Evidence routing & derived metrics | H7–H10, V2, V3, V6 | Extraction layer, precision channels, computed metrics ($\hat{s}$, TR, $\pi_i(r\mid e)$, transition metrics, $\hat{R}(c,m)$) | None — derived quantities over existing neurons; precision, never score values |
| Probes & instruments | H12, V4 | Probe formats and the three micro-probes, randomized exposure | None — instruments, like the retention probe before them |
| Rubric anchors & reporting | H15, H16, V7 | Per-dimension notes, ES event-triggering, descriptive overlay | None — the overlay is rule-based description, explicitly not a latent model |

Standing rule for future revisions: any proposed change that cannot be placed in this table's "None" column waits for the freeze lift, however good it is.
...
v0 (taxonomy) → v1 (Ritesh: neuron bank + redundancy critique + IRT) → v1.0 (full measurement instrument) → v1.1 (3-extractor pipeline + normalization + 23-chat gold + ICC + minimum-across-pillars) → v1.2 (λ + S_human + 2×2 surface) → v1.3 (judge feasibility + 3-layer sensing + retention probe) → v2.0 (unified + neuron contract schema + N/A gate + embedded state flags + flexible N-dimensions + translation layer) → **Final Compilation (this doc: 107 locked; cognitive-surrender vs cognitive-debt vocabulary [Shaw & Nave 2026]; Tri-System anchor; two-tier retention probe [Barcaui d=0.68]; three-tier claims charter; competitive positioning; algorithm-selection rationale; event-driven flowcharts)** → **Final Compilation r2 (this revision: the embedded T-reduction estimator for $S_{\text{human}}$, §5.3.1 — within-conversation autonomous-stretch baseline resolving the counterfactual bug, measurement/credit separation resolving the double-count bug, optional logprob cross-check, own assumptions + falsification test; assumption #4 upgraded from "two bugs" to "proposed method"; flowcharts 13.6 and 13.11 added/updated)** → **Final Compilation r3 (complete 107-neuron catalogue with verbatim v6.0 definitions [Appendix C] and the full 4 -> 8 -> 107 hierarchy tree — master structural tree + four per-pillar leaf-level trees [Appendix D]; §2.2 cross-referenced)** → **v2 (measurement-engine & scientific-rigor revision: the §0.0 change set — partition-guarded state conditioning, precision-not-multipliers, soft non-compensatory aggregation, n_eff scorability gate, four-rung charter, falsifiability contracts, ecology/laboratory bifurcation, maturation roadmap, rejection/deferral log + dated ontology freeze)** → **v2.1 (the humanity revision: §0.0a — humanity charter §0.6; archetype sampling frame §1.5 + quota corpus §7.2a; censored-sample principle + allocation policy §2.6 + task menus; dimension measurement notes §2.7; model-era cohorts §3.7; $A_t$ degradation branch; EC provenance/calibration/theater + calibration slope §5.8; dimension-grain annotation; generative-recall 48-h probe; twin-discrimination gate §8.5; data dignity §9.5; assumptions 6–9; freeze audit §14.4)** → **v2.2 (this doc — interaction-dynamics integration: §0.0b — universal event log; Event-Conditioned Response layer + Friction Transition Matrix + five transition metrics §5.9; descriptive regime overlay; embedded micro-probes MP-1/2/3 with randomized exposure §7.5a; partner reliability map §5.8f; commitment 9 [manufacture the counterfactual]; shared-gold bets protocol §8.6; assumptions 10–11).**
