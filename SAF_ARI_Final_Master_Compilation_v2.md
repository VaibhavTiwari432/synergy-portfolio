# Sustainable Augmentation Framework (SAF) / ARI — Final Master Compilation **(v2)**
...
**Version:** v2 — measurement-engine and scientific-rigor revision. Consolidates five rounds of convergent critique (Claude, Grok, ChatGPT, GPT-psychometrician, and the project lead) plus the four-session calibration findings. The **ontology is frozen as of this commit** (§14): no new neurons, dimensions, or pillars for the validation year. v2 changes the *measurement engine*, not the *taxonomy*.
...
**How changes are marked:** the consolidated **v2 Change Set** table below states every change (what + why + section); inline **`[v2]`** callouts mark each edit at its point of application.
