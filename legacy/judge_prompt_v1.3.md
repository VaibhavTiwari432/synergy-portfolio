# Judge Prompt v1.3 — preserved baseline (the prompt to beat)

Preserved verbatim from `chat_classifier/scorer/judge_prompt.py` at tag `v1-final`.
This is the **neuron-grain** prompt (107 individual neuron scores). The rebuild's judge
is **dimension-grain** (brief §3.6.4) — rewrite, do not extend this. The anchor examples
(EC calibration anchors, EC floor rule, AL strategic-use signal) are the reusable parts.

Calibration result with this prompt: overall MAE 0.2994 on 26 gold chats
(see `calibration_baseline_v1.3.md`).
...
`JUDGE_PROMPT_VERSION = "v1.3"` — v1.3 change: added EC floor rule (passive sessions
0.05–0.10; 0.0 only for behavior-specific neurons).
