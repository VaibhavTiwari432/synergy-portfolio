...
```
You are the Chief Engineer on a two-agent build. One junior coding agent (Codex Plus)
builds all leaf modules in parallel against contracts you freeze.
...
- You OWN all contracts, the event log, ingestion, the precision-merge layer, the
  ...
  full ownership map. Codex NEVER edits your files.
...
Do NOT build leaf modules yourself yet — those are Codex's parallel work and they
unblock the moment STAGE 0 contracts are committed. Once STAGE 0 is done and
INTERFACES.md is published, tell me, and I will release Codex.
...
## ▶ CODEX PLUS — Junior Dev A (ALL leaf modules: trait + state + dynamics)

```
You are Junior Dev A — the sole junior on a two-agent build. The Chief Engineer
(Claude Code) has frozen the contracts and published INTERFACES.md. You build
ALL leaf modules against those contracts; the Chief Engineer owns the spine —
...
contracts/ (your only imports). Skim AGENT_REBUILD_BRIEF_v3.md §3 (esp. §3.5–3.8) for
...
1. **Start Claude Code first.** Paste its block. Let it finish STAGE 0 entirely (contracts + INTERFACES.md). This is the unlock — do not start Codex before it's done, or it'll build against nothing.
2. **When Claude Code says STAGE 0 is committed:** paste the Codex block. CE and Codex now build in parallel, different files.
3. **Throttle handling:** if Claude Code hits its limit mid-STAGE-1, Codex keeps going (its work is already unblocked by frozen contracts). If Codex throttles, CE continues on spine work. Nobody picks up a throttled agent's open file — it waits.
4. **Daily:** skim DISCREPANCY.md. If Codex filed anything OPEN, that's Claude Code's first job next session. Check TEAM.md §2 boxes to see progress.
